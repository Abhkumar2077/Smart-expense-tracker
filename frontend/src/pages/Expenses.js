  ...
  const handleFormSubmit = async (formData) => {
    ...
    try {
      if (editingExpense) {
        ...
        await fetchBudgetStatus(getMonthKey(formData.date));
        showNotification('Transaction updated!', 'success');
      } else {
        const res = await expenseAPI.create(formData);
        const createdExpense = res.data?.expense || res.data;
        if (createdExpense && createdExpense.id) {
          setExpenses(prev => [createdExpense, ...prev]);
        }

        if (res.data?.alert) {
          const isExceeded = res.data?.budget_status?.status === 'exceeded';
          showNotification(
            isExceeded ? `\uD83D\uDEAB ${res.data.alert}` : `\u26A0\uFE0F ${res.data.alert}`,
